...
// ── Upravená funkce buildBarChart s tooltip ──
function buildBarChart(containerId, data) {
  ...
  items.forEach(function (item) {
    ...
    // Vlaječka a tooltip s názvem země
  ...
  function buildDailyChart(containerId, data) {
    ...
    // ── Load download stats ──
  fetch('/api/downloads.php?t=' + Date.now()) // Přidali jsme i cache-busting
    .then(function (r) { return r.json(); })
    .then(function (data) {
      var el;

      // Celkový součet (ten už v JSONu je přímo z PHP)
      el = document.getElementById('stats-dl-total');
      if (el) animateValue(el, data.total || 0);

      // Windows součet (všechny verze: x64, x86, portable x64, portable x86)
      ...
      // Linux součet (AppImage + Portable)
      ...
      // Portable celkem (všechny portable verze napříč OS)
      ...
      // Bar chart - Mapování klíčů na popisky v grafu
      var chartData = {};
